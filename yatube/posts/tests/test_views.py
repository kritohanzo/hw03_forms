...
from posts.views import Post, Group
...
class PaginatorViewsTest(TestCase):
    ...
    def test_posts_views_paginators(self):
        '''[POSTS VIEWS] Проверяем, соответствует ли ожиданиям словарь context cтраниц с паджинатором.'''
        pages = {
            'posts:index': {},
        ...
        for page, args in pages.items():
            with self.subTest(page=page):
                ...
                response = self.auth_client.get(reverse(page, kwargs=args) + '?page=2')
...
class TestPostsView(TestCase):
    ...
    def test_posts_views_use_correct_tamplates(self):
        ...
        for template, reverse_name in tamplates_page_names.items():
            with self.subTest(reverse_nam=reverse_name):
                response = self.auth_client.get(reverse_name)
                self.assertTemplateUsed(response, template)

    def test_posts_views_post_detail_context(self):
        '''[POSTS VIEWS] Проверяем, соответствует ли ожиданиям словарь context страницы post_detail.'''
        response = self.auth_client.get(reverse('posts:post_detail', kwargs={'post_id': self.post.id}))
        post_text = response.context['post'].text
        user_posts_count = response.context['posts_count']
        user_id = response.context['user']
        self.assertEqual(post_text, self.post.text, 'В контекст не передаётся текст поста')
        self.assertEqual(user_posts_count, self.post.author.posts.count(), 'В контекст не передаётся количество постов автора')
        self.assertEqual(user_id, self.post.author.id, 'В контекст не передаётся айди автора')
    
    def test_posts_views_post_edit_context(self):
        '''[POSTS VIEWS] Проверяем, соответствует ли ожиданиям словарь context страницы post_edit.'''
        response = self.auth_client.get(reverse('posts:post_edit', kwargs={'post_id': self.post.id}))
        form_object = response.context['form']
        self.assertIsInstance(form_object, PostForm, 'В контекст не передаётся форма редактирования поста')
    
    def test_posts_views_post_create_context(self):
        '''[POSTS VIEWS] Проверяем, соответствует ли ожиданиям словарь context страницы post_create.'''
        response = self.auth_client.get(reverse('posts:post_edit', kwargs={'post_id': self.post.id}))
        form_object = response.context['form']
        self.assertIsInstance(form_object, PostForm, 'В контекст не передаётся форма редактирования поста')
    
    def test_posts_views_post_create_work_correct(self):
        '''[POSTS VIEWS] Проверяем, что если при создании поста указать группу, то этот пост появляется 
