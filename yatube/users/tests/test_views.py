...
class UsersTestViewsTemplates(TestCase):
    ...
    def test_users_page_templates_without_login(self):
        '''[USERS VIEWS] Проверяем, что реверсивные urls работают с нужными шаблонами на страницах, где не нужен логин.'''
        reverse_urls = {
        ...
        for reverse_url, template in reverse_urls.items():
            with self.subTest(reverse_url=reverse_url):
                ...
                self.assertTemplateUsed(response, template, f'Reverse url {reverse_url} работает не с тем шаблоном')
    
    def test_users_page_templates_with_login(self):
        '''[USERS VIEWS] Проверяем, что реверсивные urls работают с нужными шаблонами на страницах, где нужен логин.'''
        reverse_urls = {
    ...
    def test_users_signup_context(self):
        '''[USERS VIEWS] Проверяем, что страница signup передаёт форму для создания нового пользователя в контекст в контекст.'''
        response = self.guest_client.get(reverse('users:signup'))
        self.assertIsInstance(response.context['form'], CreationForm, 'В контекст users:signup передаётся не та форма.')
